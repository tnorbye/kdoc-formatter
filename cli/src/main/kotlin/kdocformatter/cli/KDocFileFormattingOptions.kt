    ...
    fun parse(args: Array<String>): KDocFileFormattingOptions {
      ...
      while (i < args.size) {
        ...
        when {
          ...
          arg.startsWith("--single-line-comments=collapse") || arg == "--single-line-comments" ->
      ...
      if ((options.gitHead || options.gitStaged) && files.isNotEmpty()) {
        ...
        if (options.gitHead) {
          GitRangeFilter.create(options.gitPath, files.first(), false)?.let { filters.add(it) }
              ?: error("Could not create git range filter for the staged files")
        }
        if (options.gitStaged) {
          GitRangeFilter.create(options.gitPath, files.first(), true)?.let { filters.add(it) }
